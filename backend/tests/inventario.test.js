...
describe('Inventario Controller', () => {
  ...
  describe('POST /inventario/productos', () => {
    ...
    test('Debería rechazar producto con referencia inválida (caracteres especiales)', async () => {
      ...
      const productoData = {
        referencia: 'TEST-@Invalid', // @ no es permitido por regex y no es eliminado por sanitizer
  ...
  describe('POST /inventario/movimientos', () => {
    test('Debería crear un movimiento válido', async () => {
      ...
      // Crear SIEMPRE un nuevo producto para este test
      const producto = await Product.create({
...
describe('Validaciones de Seguridad', () => {
  test('Debería sanitizar inputs contra XSS', async () => {
    ...
    // El sanitizer limpia el input, así que el producto SE CREA (201) pero limpio
    expect([200, 201]).toContain(res.status);

    // Verificar que se guardó sanitizado
    if (res.body.data) {
      expect(res.body.data.nombre).not.toContain('<script>');
      expect(res.body.data.nombre).toContain('Producto');
    }
  });
